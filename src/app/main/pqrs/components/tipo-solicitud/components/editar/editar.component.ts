...
export class EditarComponent implements OnInit {
  ...
  submit(){
    ...
    try {
      this.pqrsService.put(`${this.API_URI}/pqrs/tipo/update/${this.id}/`,this.form.value, this.token).subscribe( respuesta => {
        this.form.reset();
        this.traerSolicitudes();
        this.showEditar('');
  ...
  traerSolicitudes(){
    ...
    try {
      this.pqrsService.get(`${this.API_URI}/pqrs/tipo/`, this.token).subscribe( respuesta => {
        ...
        this.tipo_solicitudes =  respuesta.data;
        respuesta.data.map( (solicitud:any) => this.tipo_solicitudesVerificated.push(solicitud.tipo.toLowerCase().trim()));
      })
    } catch (error) {
      console.log(error)
