...
export class EditarComponent implements OnInit {
  ...
  submit() {
    ...
    try {
      this.momentosService.put(`${this.API_URI}/poll/momentos/update/${this.id}/`, this.form.value, this.token).subscribe(respuesta => {
  ...
  traerMomentos() {
    this.momentos = [];
    this.momentosTipo = [];
    this.momentosService.get(`${this.API_URI}/poll/momentos/`, this.token).subscribe(respuesta => {
      respuesta.data.map((momento: any) => this.momentosTipo.push(momento.tipo.toLowerCase()))
      respuesta.data.map((momento: any) => this.momentos.push(momento))
