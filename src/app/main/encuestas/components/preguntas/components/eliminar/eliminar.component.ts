...
export class EliminarComponent implements OnInit {
  ...
  traerPreguntas() {
    this.preguntas = [];
    try {
      this.preguntasService.get(`${this.API_URI}/poll/questions/`, this.token).subscribe(respuesta => this.preguntas = respuesta.data)
    } catch (error) {
      console.log(error)
    }
  }
  ...
  confirm(event: Event | any, id:any) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        try {
          this.preguntasService.delete(`${this.API_URI}/poll/questions/delete/${id}/`, this.token).subscribe(respuesta => {
