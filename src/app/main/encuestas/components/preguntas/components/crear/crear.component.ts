...
export class CrearComponent implements OnInit {
  ...
  onSubmit() {
    ...
    // console.log(body)
    try {
      this.preguntasService.post(`${this.API_URI}/poll/questions/create/`, body, this.token).subscribe(respuesta => {
        this.form.reset();
  ...
  traerMomentos() {
    try {
      this.preguntasService.get(`${this.API_URI}/poll/momentos/`, this.token).subscribe(respuesta => {
