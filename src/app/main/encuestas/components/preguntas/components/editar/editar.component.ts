...
export class EditarComponent implements OnInit {
  ...
  traerMomentos(){
    ...
    try {
      this.preguntaService.get(`${this.API_URI}/poll/momentos/`, this.token).subscribe(respuesta => {
  ...
  submit() {
    ...
    try {
      this.preguntaService.put(`${this.API_URI}/poll/questions/update/${this.idPregunta}/`, body, this.token).subscribe(respuesta => {
        this.form.reset();
        this.traerPreguntas();
        this.traerMomentos();
