...
export class EditarComponent implements OnInit {
  ...
  submit() {
    ...
    try {
      this.clasificadosService.put(`${this.API_URI}/advertisements/category/update/${this.id}/`, this.form.value, this.token).subscribe(r => {
        this.traerCategorias();
        this.form.reset();
        this.display = false;
  ...
  traerCategorias() {
    this.categorias = [];
    this.categoriasVerificated = [];
    try {
      this.clasificadosService.get(`${this.API_URI}/advertisements/category/`, this.token).subscribe(r => {
        this.categorias = r.data;
        r.data.map((seccion: any) => this.categoriasVerificated.push(seccion.name.trim().toLowerCase()));
      })
    } catch (error) {
      console.log(error)
    }
