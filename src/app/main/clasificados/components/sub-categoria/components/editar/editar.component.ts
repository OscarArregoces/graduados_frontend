...
export class EditarComponent implements OnInit {
  ...
  submit() {
    ...
    try {
      this.clasificadosService.put(`${this.API_URI}/advertisements/sub/category/update/${this.id}/`, this.form.value, this.token).subscribe(r => {
        this.traerSubCategorias();
        this.form.reset();
        this.display = false;
  ...
  traerSubCategorias() {
    ...
    try {
      this.clasificadosService.get(`${this.API_URI}/advertisements/sub/category/`, this.token).subscribe(r => {
        this.subCategoria = r.data;
        r.data.map((seccion: any) => this.subCategoriaVerificated.push(seccion.name.trim().toLowerCase()));
      })
    } catch (error) {
      console.log(error)
    }
