...
export class CrearComponent implements OnInit {
  ...
  onSubmit() {
    ...
    let body = {
      ...
      categoriaId: this.form.value.categoriaId.id
    }

    // console.log( this.form.value )

    try {
      this.clasificadosService.post(`${this.API_URI}/advertisements/sub/category/create/`, body , this.token).subscribe(r => {
        this.form.reset();
        console.log(r)
  ...
  traerSubCategorias() {
    ...
    try {
      this.clasificadosService.get(`${this.API_URI}/advertisements/sub/category/`, this.token).subscribe(r => {
        r.data.map((seccion: any) => this.subCategorias.push(seccion.name.trim().toLowerCase()))
      });
    } catch (error) {
      console.log(error)
    }
